...
export class AdminDialogAddOrModifyHaloCheck extends React.Component {
  ...
  onSubmit() {
    ...
    onDone({ ...this.state, event_id: parseInt(this.props.event.id) })
  }
    ...
    return (
      <div className="admin-dialog-halo-check">
        <div className="admin-dialog-top-header">
          <AdminTitle>Create a new check</AdminTitle>
          ...
          <AdminFormRow size={0}>
            <AdminFormColumn>
              ...
              <div className="admin-dialog-halo-check__tags">
                ...
                <TagInput
                  placeholder="Press enter to submit zone"
                  tags={data.zones}
                  suggestedTags={
                    zones
            ...
            <div className="admin-form__image-field">
              <AdminFormColumn>
                ...
                <div className="admin-dialog-halo-check__assignees">
                  <div className="admin-dialog-halo-check__assignees__svg-search">
        ...
        <p className="muted-bottom">
          {data.recurringPeriod !== RECURRING_PERIOD.Never ? (
            this.isEndDateValid() && this.isEndDateAfterStartDate() ? (
              <>
                This check will recur every&nbsp;
                {data.recurringPeriod === RECURRING_PERIOD.EveryFifteenMinutes
                  ? '15 minutes'
                  : null}
                {data.recurringPeriod === RECURRING_PERIOD.EveryThirtyMinutes
                  ? '30 minutes'
                  : null}
                {data.recurringPeriod === RECURRING_PERIOD.Hourly
                  ? 'hour'
                  : null}
                {data.recurringPeriod === RECURRING_PERIOD.Daily ? 'day' : null}
                ...
                &nbsp;Checks will be visible to app users 1 hour before they are
                due.
              </>
            ) : null
          ) : (
            <>
              This check will be visible to app users 1 hour before they are
              due.
            </>
          )}
        </p>
