...
const AdminDocumentSelector = props => {
  ...
  const filteringData = (search, filter) => {
    ...
    if (search !== '' && filter !== '') {
      let temp = tempDocument.filter(
        doc => doc.docName.match(search) && doc.eventId === filter,
      )
      setLocalList(temp)
    } else if (search !== '') {
      ...
      {loading ? (
        ...
        localList && (
          <AdminTableBody
            headers={
              eventId === 'global'
                ? ['Documents', 'Upload Date', 'Assign role', 'Event', 'Other']
                : ['Documents', 'Upload Date', 'Assign role']
            }
            selectedRowIds={selectedDocumentIds}
            columns={
              eventId === 'global'
                ? ['docName', 'date', 'assignRole', 'eventId', 'edit']
                : ['docName', 'date', 'assignRole', 'edit']
            }
            onRowCheckboxChange={(row, value) =>
              setSelectedDocumentIds(
                getNewCheckboxList(selectedDocumentIds, row.id, value),
              )
            }
            data={localList}
            showCheckboxes
                ...
                ? [
                    {
                      ...
                      render: check => (
                        <button
                          ...
                          onClick={() => handleDownloadDocument(check)}
                        >
                          {check.docName}
                        </button>
                      ),
                    ...
                    },
                    {
                      column: 'eventId',
                      render: check => {
                        let found = props.events.find(
                          x => x.object_id === check.eventId,
                        )
                        if (found) {
                          return found.title
                        }
                    ...
                    },
                    {
                      ...
                      render: check => (
                        <span
                          ...
                          onClick={() => handleEditDocument(check)}
                        >
                          <SVGIcons name={'editBlack'} />
                        </span>
                        // <button
