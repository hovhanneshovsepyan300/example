...
const IncidentTable = ({
    ...
    for (let incident of incidents) {
      ...
      let type = (
        <div className="incident-type">
          <SVGIcons name={incident.type_value} height={20} width={20} />
          <div className="incident-type__name">
      ...
      let preparedIncidents = {
        ...
        status: status,
        statusOrder: status?.props?.badge?.label,
        userStatus,
        rowBgColor,
      }
  ...
  const getStatus = incident => {
    let status = <Badge badge={INCIDENT_STATUS.New} width={'60px'} />
    if (incident.resolved)
      status = <Badge badge={INCIDENT_STATUS.Resolved} width={'60px'} />
    if (incident.archived)
      status = <Badge badge={INCIDENT_STATUS.Archived} width={'60px'} />
  ...
  const hasUpdate = incident =>
    utils.isIncidentHasUpdates(incident, currentUser)
...
export default compose(
  ...
  connect(state => {
    ...
    console.log('INCIDENTS TABLE DATA', incidentsTableData)
