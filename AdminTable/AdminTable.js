...
class AdminTable extends Component {
  ...
  componentDidMount() {
    ...
    const calculatedRowPerPage = Math.floor((height - 59) / 32)

    // this is ok
    // eslint-disable-next-line react/no-did-mount-set-state
    this.setState({ calculatedRowPerPage })
    console.log('Log just for open PR')
    // @-ADD-TO-DO remove this after successfull PR
  }
  ...
  goToSelectedPage(page) {
    ...
    this.props.handlePagination({ page: currentPage })
  ...
  onPagination = direction => () => {
    ...
    if (direction === 'prev' && !currentPage) return

    if (direction === 'next') {
      currentPage = currentPage + (!currentPage ? 2 : 1)
    } else {
    ...
    if (
      (direction === 'next' && currentPage && filteredData.length) ||
      (direction === 'prev' && currentPage)
    ) {
      handlePagination({ page: currentPage, direction })
  ...
  render() {
    ...
    return (
      <div
        className={
          incidentTable
            ? `incident-detail-table incident-detail-table--${variant}`
            : `admin-table admin-table--${variant}`
        }
      >
        {/* <AdminTableHeader
        ...
        />
        {!incidentTable && !this.props && filteredData.length && (
          <AdminTableFooter
            currentPage={
            ...
            //     : pageCount
            // }
            // totalPageCount={currentPage || currentPage + 1}
            totalPageCount={this.props.totalPageCount}
            // onPreviousPageClick={() => this.goToPreviousPage()}
            onPreviousPageClick={this.onPagination(PAGINATION_MODES.prev)}
            // onNextPageClick={() => this.goToNextPage()}
            onNextPageClick={this.onPagination(PAGINATION_MODES.next)}
